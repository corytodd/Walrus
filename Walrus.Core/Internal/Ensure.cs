namespace Walrus.Core.Internal
{
    ...
    internal static class Ensure
    {
        ...
        {
            var instance = Activator.CreateInstance(typeof(TException), message);
            if (instance is not TException exception)
            {
                var innerEx = new Exception($"Name={name}, message={message}");
                var outerEx = new Exception($"Failed to create Exception {typeof(TException).Name}", innerEx);
                return outerEx;
            }
